...
个人单机、单用户、多会话的 Web 终端 coding agent。用户可从顶栏打开**交互型**子 agent（侧栏对话）；主 agent 可持续工作，并按需派生**工作型**（后台并发）子 agent。文件与命令默认走用户审批。
...
- 交互型子 agent：顶栏手动打开侧栏对话，结束后摘要回投主 agent
...
4. 顶栏「子 Agent」打开交互型侧栏对话（关侧栏 ≠ 终止）。主 agent 可派生工作型（底部工作区）。
