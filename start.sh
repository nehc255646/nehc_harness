...
(
  ...
  exec uv run uvicorn app.main:app --host 0.0.0.0 --port "$BACKEND_PORT" --workers 1
) >"$LOG_DIR/backend.log" 2>&1 &
BACKEND_PID=$!

# ---------- 前端 ----------
log "启动前端 (vite dev --host)..."
(
  cd "$ROOT/frontend"
...
if [ -n "${DISPLAY:-}" ] || [ -n "${WAYLAND_DISPLAY:-}" ]; then
  ...
  warn "VM 内无图形环境，请在 Windows 浏览器访问:"
  log "  http://localhost:$FRONTEND_PORT  (本机)"
  [ -n "$VM_IP" ] && log "  http://$VM_IP:$FRONTEND_PORT    (Windows 侧，需 ufw 放行 5173/8000)"
fi
